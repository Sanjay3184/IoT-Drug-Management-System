...
DHT dht(DHT11PIN, DHT11); //Create Dht11 instance
...
String Web_App_URL1 = "https://script.google.com/macros/s/AKfycbxQl3zGtUh3h3gnIDoIu3Y_pmH02NqGALWDNL8DMU4TBu_HhG2HxoaPDziM2cTATc8m/exec";
...
void loop() {
  ...
  String Send_Data1_URL = Web_App_URL1 + "?sts=write";
  Send_Data1_URL += "&temp=" + String(temp);
  Send_Data1_URL += "&humi=" + String(humi);
  ...
  // Initialize HTTPClient as "http1".
  HTTPClient http1;
  
  Serial.print("-------------");
  Serial.println("Send data to Google Spreadsheet...");
  Serial.print("URL : ");
  Serial.println(Send_Data1_URL);

  http1.begin(Send_Data1_URL.c_str());
